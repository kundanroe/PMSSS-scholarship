...
$doc1=$_FILES["reqdoc1"]["name"];
$doc2=$_FILES["reqdoc2"]["name"];
$extension = substr($pic,strlen($pic)-4,strlen($pic));
$extension1 = substr($doc,strlen($doc)-4,strlen($doc));
$extension2 = substr($doc1,strlen($doc1)-4,strlen($doc1));
$extension3 = substr($doc2,strlen($doc2)-4,strlen($doc2));
...
if(!in_array($extension2,$allowed_extensions1))
{
echo "<script>alert('File has Invalid format. Only docs / doc/ pdf format allowed');</script>";
}
if(!in_array($extension3,$allowed_extensions1))
{
echo "<script>alert('File has Invalid format. Only docs / doc/ pdf format allowed');</script>";
}
else
...
$doc1=md5($doc1).time().$extension2;
$doc2=md5($doc2).time().$extension3;
 move_uploaded_file($_FILES["pic"]["tmp_name"], "proimages/$pic");
 move_uploaded_file($_FILES["reqdoc"]["tmp_name"], "document/$doc");
 move_uploaded_file($_FILES["reqdoc1"]["tmp_name"], "document/$doc1");
 move_uploaded_file($_FILES["reqdoc2"]["tmp_name"], "document/$doc2");
...
$sql="insert into tblapply(SchemeId,ApplicationNumber,UserID,DateofBirth,Gender,Category,Religion,Address,AadharNumber,ProfilePic,DocReq,DocReq1,DocReq2)values(:schemeid,:appnum,:uid,:dob,:gender,:category,:religion,:address,:adharno,:pic,:doc,:doc1,:doc2)";
...
$query->bindParam(':doc1',$doc1,PDO::PARAM_STR);
$query->bindParam(':doc2',$doc2,PDO::PARAM_STR);
 $query->execute();
...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
 ...
 <table class="table table-bordered table-hover data-tables">
  ...
  <tr>
    <th style="color: blue;">Upload Required Doc1 :</th>
    <td>
    <input type="file" name="reqdoc1" value="" class="form-control" required="true" style="border:solid 1px #000;color:#000;"> </td>
  </tr> 
  <tr>
    <th style="color: blue;">Upload Required Doc2 :</th>
    <td>
    <input type="file" name="reqdoc2" value="" class="form-control" required="true" style="border:solid 1px #000;color:#000;"> </td>
  </tr> 
  <tr>
